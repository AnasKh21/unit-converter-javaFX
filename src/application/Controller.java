...
public class Controller implements Initializable //, ChangeListener<String>
{
	...
	public void initialize(URL location, ResourceBundle resources)
	{
		 ...
		 */
		//Pour le try catch, je vous en réfère à votre cours.
		try {
			//C'es totalement arbitraire, vous pouvez mettre ce que vous voulez en MesureType
			this.converter=new Converter(MeasureType.LENGTH);
		} catch (ParseException e) {
			throw new RuntimeException(e);
		}
		//On récupère toutes les sources du converter et on les mets dans la liste déroulante de Comboboc
		sourceUnitComboBox.setItems(converter.getSourceUnits());
		//Même chose qu'au dessus
		destinationUnitComboBox.setItems(converter.getdestinationUnits());
		//Set la valeur en cours, dans le cas présent , notre millimétre (je crois)
		sourceUnitComboBox.setValue(converter.getSourceUnit());
		destinationUnitComboBox.setValue(converter.getDestinationUnit());
		//Différentes bind de propriétes pour maj rapide
		//C'est long à expliquer, j'ai juste ressorti le projet de l'année dernière pour comprendre ça
