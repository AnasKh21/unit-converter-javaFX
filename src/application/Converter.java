...
public class Converter
{
	...
	public void applyMeasureType() throws ParseException
	{
		 ...
		 */
		//J'ai suivi le TODO, ouais je sais, c'est impressionnant
		clear();
		sourceUnits.clear();
		destinationUnits.clear();
		Set<Unit<Double>> Vader=UnitsFactory.getUnits(this.measureType.getValue());
		for(Unit<Double> e : Vader){
			if (e.isSetable()){
		...
		//this.sourceUnits.sort(this.sourceSortOrder.getValue());
		//this.destinationUnits.sort(this.destinationSortOrder.);
		selectFirstUnits();
	}
...
//			logger.info("Unexchangeable property set to " + unexchangeableUnits.get());
	...
	public void convert() throws IllegalStateException
	{
		 ...
		 */
		//Un concentré de if Tout à fait passionnant
		Unit<Double> source = sourceUnit.get();
	...
	public void switchUnits()
	{
		 ...
		 */
		//Vous trompez pas dans l'ordre, ça casse tout sinon
		Unit<Double> source = sourceUnit.get();
		if(source==null){
		...
		if(this.unexchangeableUnits.get()){
			//Soyez plus original
			logger.warning("Non, je suis ton père !");
			logger.warning("Non, ce n'est pas vrai , c'est impossible !");
		}
		else{
			destinationUnit.set(source);
	...
	public void clear()
	{
		 ...
		 */
		if(!this.inputText.isBound()){
			//Vous pouvez mettre null aussi, mais moi ça voulait pas marcher
			this.inputText.set("");
		}
		Unit<Double> source = sourceUnit.get();
		//Le if not null, un grand classique
		if(source!=null){
		if(source.hasValue()){
			this.sourceUnit.getValue().clearValue();
		}}
		// ...
		if(!this.outputText.isBound()){
			this.outputText.set("");
		}
		//oui c'est répétitif !
		Unit<Double> destination = destinationUnit.get();
		...
		if(destination.hasValue()){
			this.destinationUnit.getValue().clearValue();
		}}
		// ...
	}
